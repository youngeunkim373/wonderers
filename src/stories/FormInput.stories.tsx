...
import { FormInput, FormInputWithButton, FormInputInvalid, FormInputPassword } from '.';
...
export const FormInputWithButtonStory = () => {
  const onHandleSubmit = () => alert('중복 체크');

  return (
    <FormInputWithButton
      label="레이블"
      inputProps={{ type: 'text', placeholder: '값을 입력하세요' }}
      buttonProps={{ children: '중복 체크', onClick: onHandleSubmit }}
    />
  );
};
...
export const FormInputInvalidStory = () => {
  return (
    <FormInputInvalid
      label="이메일 주소"
      inputProps={{ type: 'email', placeholder: '이메일 주소를 입력해 주세요.' }}
